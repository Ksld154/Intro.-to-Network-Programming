...
sqlite_db = pw.SqliteDatabase('0516215_Project4.db', pragmas={'foreign_keys': 1})
    ...
    def Login_User(self, account, password):
        try:
            ...
            if query.count() == 1:
                ...
                q3 = Subscribe.select().where(Subscribe.user_id == query[0].id)
                topics = []
                for t in q3:
                    topics.append(t.Group.group_name)

                # res = [t.username for t in q2]

                json_file = json.dumps({'status':0, 'token':token, 'message': 'Success!', 'subscribed_topics':topics}, indent=2)
                return json_file
...
class Group(SQLiteModel):
    id = pw.IntegerField(primary_key=True)
    group_name = pw.CharField(unique=True)

class Subscribe(SQLiteModel):
    id = pw.IntegerField(primary_key=True) 
    user_id  = pw.ForeignKeyField(User,  on_update='CASCADE', on_delete='CASCADE')
    group_id = pw.ForeignKeyField(Group, on_update='CASCADE', on_delete='CASCADE')
